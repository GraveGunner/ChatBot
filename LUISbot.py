...
class LUISBot(ActivityHandler):
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        # await turn_context.send_activity(f"You said {msg}") # Used to Echo
        analysis = self.luis_model.analyze(msg)
        intent_luis = analysis.best_intent()
        await turn_context.send_activity(f"Top Intent : {intent_luis.intent}")
        for entity in analysis.entities:
            await turn_context.send_activity(f"Entity : {entity.entity}")
            await turn_context.send_activity(f"Entity Type : {entity.type}")
